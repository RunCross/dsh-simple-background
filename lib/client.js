window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var RAW_STYLE_ID = 'dsh-custom-css-raw'
    var BG_STYLE_ID = 'dsh-custom-css-bg'
    var API_STYLE_ID = 'dsh-custom-css-api'
    var UI_STYLE_ID = 'dsh-custom-css-ui'
    var THEME_STYLE_ID = 'dsh-custom-css-theme'
    var LAYOUT_STYLE_ID = 'dsh-custom-css-layout'
    var SIDE_STAGE_ID = 'dsh-custom-css-side-stage'
    var SIDE_STYLE_ID = 'dsh-custom-css-side-style'
    var BG_LAYER_ID = 'dsh-custom-css-bg-layer'
    var SIDEBAR_BG_STYLE_ID = 'dsh-custom-css-sidebar-bg'
    var SCRIPT_TAG_ID = 'dsh-custom-css-user-script'
    ...
    var SIDE_CSS = [
      '[data-dsh-custom-css-side-stage]{position:fixed;inset:0;z-index:0;pointer-events:none;overflow:hidden;contain:strict}',
      '[data-dsh-custom-css-side]{position:absolute;display:block;object-fit:contain;object-position:center bottom;width:auto;max-width:none;filter:drop-shadow(0 18px 26px rgba(15,23,35,.28));transition:opacity .2s}',
      '[data-dsh-custom-css-side="left"]{left:var(--dsh-cc-sidebar-width, 0px)}',
      '[data-dsh-custom-css-side="right"]{right:var(--dsh-cc-details-width, 0px)}',
      'body[data-ds-dark-theme] [data-dsh-custom-css-side]{filter:brightness(.86) saturate(.92) drop-shadow(0 20px 28px rgba(0,0,0,.32))}'
    ].join('\n')
    ...
    function ensureStyle(id) {
      ...
      if (!el) {
        ...
        el.id = id
        el.setAttribute('data-plugin', 'dsh-custom-css')
    ...
    function applySideImage(stage, side, cfg) {
      ...
      var selector = '[data-dsh-custom-css-side="' + side + '"]'
      ...
      if (!img) {
        ...
        img.setAttribute('data-dsh-custom-css-side', side)
        img.alt = ''
    ...
    function nodeTouchesLayoutColumns(node) {
      ...
      if (node.getAttribute && node.getAttribute('data-plugin') === 'dsh-custom-css') return false
    ...
    function applySideImages(s) {
      ...
      if (!stage.querySelector('[data-dsh-custom-css-side]')) {
        removeSideStage()
        teardownLayout()
        return
      }
    ...
    function apply(ctx) {
      ...
      ctx.effect(function () { return ctx.locale.register(LOCALE_NS, { zh: zh, en: en }) }, 'dsh-custom-css: dictionaries')
      ...
      ctx.effect(function () { return currentScope.subscribe(sync) }, 'dsh-custom-css: settings sync')
      sync()
