...
export function apply(ctx) {
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    settingsCtx.effect(() => scope.watch((next) => {
      try {
        ...
        settingsCtx.logger?.warn?.('dsh-custom-css: failed to write user script: ' + message)
      }
    }), 'dsh-custom-css: write user script')
  })
  ...
  ctx.inject(['webServer'], (httpCtx) => {
    ...
    }), 'dsh-custom-css: user script route')
  })
}
